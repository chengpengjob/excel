...
import com.cp.constant.ExcelFormat;
import com.cp.entity.ExcelHeaderInfo;
import com.cp.entity.po.TtlProductInfoPo;
import com.cp.mapper.TtlProductInfoMapper;
import com.cp.service.TtlProductInfoService;
import com.cp.utils.ExcelUtils;
...
/**
 * @author ChengPeng
 * @create 2019-11-06 10:01
 */
@Service
@Slf4j
public class TtlProductInfoServiceImpl implements TtlProductInfoService {
    private static final Logger LOGGER = LoggerFactory.getLogger(TtlProductInfoServiceImpl.class);
    //每个线程导出记录最大行数
    ...
    private List<TtlProductInfoPo> multiThreadListProduct() {
        ...
        LOGGER.info("多线程查询，总数：{},开启线程数：{}", totalNum, loopNum);
    ...
    private void executeTask(List<FutureTask<List<TtlProductInfoPo>>> tasks, int loopNum, int total) {
        for (int i = 0; i < loopNum; i++) {
            ...
            LOGGER.info("开始查询第{}条开始的{}条记录", i * THREAD_MAX_ROW, THREAD_MAX_ROW);
